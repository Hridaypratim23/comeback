...
export default function WorkoutPage() {
  const { dayLogs, markWorkoutDone, toggleExerciseCheck, setWorkoutNotes, getOrCreateToday } = useStore()
      ...
      <div className="px-4 space-y-4">
        ...
        <div className="flex items-start justify-between pt-8">
          <div className="flex-1">
            <p className="text-[10px] font-black tracking-[0.35em] text-[#FF2800]">MISSION</p>
            ...
            <p className="text-[10px] italic text-[#2C2C38] mt-1">"{getQuoteOfHour()}"</p>
          </div>
          <button
          ...
          <>
                ...
                return (
                  <button key={ex.id} onClick={() => toggleExerciseCheck(ex.id)}
                    className={`w-full flex items-center gap-3 p-4 rounded-xl border transition-all cursor-pointer btn-press text-left
                      ${done
                        ? 'bg-[#0D7A3A22] border-[#1DB95444]'
                        : 'bg-[#111116] border-[#1E1E26] hover:border-[#2C2C38]'}`}>
            ...
            {!workoutDone ? (
              <button
                ...
                {doneCount === 0 ? 'CHECK OFF EXERCISES TO COMPLETE' : `MISSION COMPLETE · +100 XP`}
              </button>
            ) : (
